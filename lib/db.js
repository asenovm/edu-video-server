...
exports.insertTags = function (tags, callback) {
    ...
    connectToDb(function (db) {
        ...
        tags.forEach(function (tag) {
            tagsCollection.insert({ name: tag.name }, function (err, tags) {
                --tagsLeft;
                if(!tagsLeft) {
                    callback(err, tags);
                    db.close();
...
exports.retrieveVideos = function (tags, callback) {
    connectToDb(function (db) {
        var videosCollection = db.collection('videos'),
            tagNames = _.pluck(tags, 'name');
        videosCollection.find({tags: { $in : tagNames }}).toArray(function (err, videos){
            callback(err, { videos : videos});
        });
    });
};
